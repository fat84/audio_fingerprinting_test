...
void ofApp::update(){
    ...
    if( predicting && !tThresholdMode){
        if( pipeline.predict( inputVector ) ){
            ...
            fillVector(predictedClassLabel);
...
void ofApp::draw(){
    ...
    if( pipeline.getTrained() ){
        ...
        hugeFont.drawString( txt1, 32, 200);
...
void ofApp::fillVector(int predictedClassLabel) {
    classification_buffer.push_back(predictedClassLabel);
...
void ofApp::sendOSC (){
    ...
    ofLog(OF_LOG_NOTICE, "Sending prediction over OSC " + ofToString(prediction));
    ofLog(OF_LOG_NOTICE, "Sending rms over OSC" + ofToString(rms));
    
    ofxOscMessage m;
    m.setAddress(oscAddress);
    m.addIntArg(prediction);
    m.addFloatArg(rms);
...
int ofApp::mostCommon(vector<int> a){
    
    int max=0, mostvalue=a[0];
        
    for(int i=0; i < a.size(); i++ ) {
            int co = (int)count(a.begin(), a.end(),a[i]);
            if (co > max) {
                max = co;
                mostvalue = a[i];
            }
        ...
        //cout << "Most Common" << mostvalue << endl;
    return  mostvalue;
